...
Перед розгортанням образу:

- застосовує версію релізу;
- виконує перевірки TypeScript, lint, форматування, збірки, тестування та покриття;
- створює локальний пробний образ релізу;
...
- запускає `--help` в образі;
- порівнює образ із вихідним Playwright MCP за допомогою скрипта перевірки відповідності
  bridge;
- завантажує звіт про відповідність мосту у форматі JSON як артефакт робочого процесу;
- сканує образ за допомогою Trivy на наявність високих та критичних вразливостей ОС та бібліотек.
...
`linux/arm64`. У рамках робочого процесу випуску проводиться попереднє тестування обох платформ перед публікацією
та підтримується порівняння сумісності браузерів для `linux/amd64`.
...
GHCR завершується до початку публікації в реєстрі. Ця складена
...
`docs-build` та `docs-deploy` розгортають MkDocs за допомогою вбудованого потоку розгортання GitHub Pages Actions
. У налаштуваннях сторінок репозиторію як джерело слід вказати `GitHub Actions`
.
...
`actions/deploy-pages` у середовище `github-pages`.
