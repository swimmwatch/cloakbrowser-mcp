...
Перад зліццём release PR CI правярае:

- запускае праверкі TypeScript, lint, format, build, test і coverage;
- правярае метаданыя і змесціва npm-пакета;
- збірае Docker-вобразы для `linux/amd64` і `linux/arm64`;
- запускае smoke-праверкі Docker `--help`;
- параўноўвае вобраз `linux/amd64` з upstream Playwright MCP з дапамогай скрыпта
  праверкі парытэту моста;
- скануе Docker-вобразы з дапамогай Trivy на наяўнасць сур'ёзных і крытычных
  уразлівасцей АС/бібліятэк.

Падчас публікацыі рэлізу Docker workflow:
...
- выдаляе невыкарыстаны глабальны npm-пайлоуд з runtime-вобраза;
- публікуе шматплатформавы вобраз;
- абнаўляе агляд Docker Hub пасля паспяховага push вобраза.
...
`linux/arm64`. PR CI выконвае smoke-праверкі абедзвюх платформ перад зліццём і падтрымлівае параўнанне парытэту браузерных інструментаў для `linux/amd64`.
...
Праца рэестра MCP пачынаецца з той жа падзеі выпуску ў GitHub, што і npm, Docker і публікацыя дакументацыі. Яна дэкларуе `needs: [npm, docker]`, таму публікацыя npm і Docker завяршаецца, перш чым пачнецца публікацыя ў рэестры. Разгортванне дакументацыі дэкларуе `needs: [docs-build, npm, docker, mcp-registry]`, таму GitHub Pages абнаўляецца толькі пасля паспяховай публікацыі npm, Docker і афіцыйнага MCP Registry. Складаная
...
Заданні `docs-build` і `docs-deploy` разгортваюць MkDocs з дапамогай убудаванага працэсу разгортвання GitHub Pages Actions. У наладах Pages рэпазіторыя павінен выкарыстоўвацца `GitHub Actions` у якасці
крыніцы.
...
`actions/deploy-pages` у асяроддзе `github-pages` толькі пасля паспяховай публікацыі npm,
